...
$UV run python -m moelar.train.extract --model $M4 --records $RECORDS \
...
$UV run python -m moelar.train.residual --train data/features-full/train.npz --heldout data/features-full/heldout.npz \
  --epochs 10 --out checkpoints/pointer_head_v2.npz > logs/train-4b-v2.log 2>&1
$UV run python -m moelar.train.report --shard data/features-full/test.npz --head checkpoints/pointer_head_v2.npz \
...
$UV run python -m moelar.train.extract --model $M9 --records $RECORDS \
  --test-records evals/data/*.test.jsonl --test-limit 1100 --limit 20000 --holdout-fraction 0.2 --max-options 160 \
  --out data/features-9b > logs/extract-9b.log 2>&1
step "train-9b"
$UV run python -m moelar.train.residual --train data/features-9b/train.npz --heldout data/features-9b/heldout.npz \
  --epochs 10 --out checkpoints/pointer_head_9b.npz > logs/train-9b.log 2>&1
$UV run python -m moelar.train.report --shard data/features-9b/test.npz --head checkpoints/pointer_head_9b.npz \
