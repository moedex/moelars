...
$UV run python -m moelar.train.lora --model $M4 --records $RECORDS --limit 400 --eval-every 50 \
  --out checkpoints/lora-probe > logs/lora-probe.log 2>&1
step "lora-4b: full corpus, one epoch"
$UV run python -m moelar.train.lora --model $M4 --records $RECORDS --limit 20000 \
...
$UV run python -m moelar.train.extract --model $M4 --adapter checkpoints/lora-4b --records $RECORDS \
