...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    print(f"MoeLAR {__version__} serving {engine.model_id} on http://{args.host}:{args.port}", file=sys.stderr)
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    from moelar.evalset import calibrate, read_examples
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="moelar", description="Moe Limited but Accurate Response")
    parser.add_argument("--version", action="version", version=f"moelar {__version__}")
